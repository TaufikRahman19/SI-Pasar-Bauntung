            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <form action="{{ route('bayarTunggakan') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-sm-6">
                                        {{-- <div class="form-group">
                                            ...
                                            <input type="hidden" class="form-control form-control" id="pegawai_id" name="pegawai_id" value="{{ Auth::user()->name}}" readonly>
                                        </div> --}}
                                        <div class="form-group">
                                            ...
                                            <select readonly class="form-control form-control" name="pedagang_id" id="pedagang_id" onchange="autofill()"
                                                ...
                                                @foreach ($pedagangs as $pedagang)
                                                    <option value="{{ $pedagang->id }}" {{ $pedagang->nama ==  Auth::user()->name  ? 'selected' : '' }} value="{{ $pedagang->id }}" >{{ $pedagang->nama }} - {{ $pedagang->no_register }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Jumlah Bulan</label>
                                            <input type="number" value="{{ $pedagang->tunggakan->bulan }}" readonly name="perbulan" id="perbulan" class="form-control form-control" required>
                                        </div>
                                        <div class="form-group">
